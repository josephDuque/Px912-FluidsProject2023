...
real(real64) :: G, nu, volumeFlux_ana, volumeFlux_num
...
real(real64), dimension ( n ) :: y, f, temp, source, f_ana, integrand, absErr, relErr
...
!timeStep = (0.1*h**2)/nu
totalTimeSteps = floor(totalSimTime/timeStep)+1
!---------------------------------------------------------
...
do i =1 , n
  ...
  f_ana(i) = G/(2.0_real64*nu)*(W**2 - y(i)**2 + 2.0_real64*W*l)
end do
!---------------------------------------------------------
...
do i = 2 ,n-1
  A(i, i-1) = (timeStep*nu)/(h**2)
  A(i, i) = 1.0_real64 - (2.0_real64*timeStep*nu)/(h**2)
  A(i, i+1) = (timeStep*nu)/(h**2)
  source(i) = (timeStep*G) 
end do
...
A(1, 1) = 1.0_real64 - (2.0_real64*timeStep*nu)/(h**2) - (2.0_real64*timeStep*nu)/(h*l)
A(1, 2) = (2.0_real64*timeStep*nu)/(h**2)
source(1) = timeStep*G

A(n, n-1) = (2.0_real64*timeStep*nu)/(h**2)
A(n, n) = 1.0_real64 - (2.0_real64*timeStep*nu)/(h**2) - (2.0_real64*timeStep*nu)/(h*l)
source(n) = timeStep*G
!!---------------------------------------------------------
...
! numeric volume flux:
!volumeFlux_ana = (2.0_real64*(W**3)*G)/(3.0_real64*nu) * (1.0_real64 + (4.0_real64*l)/W)
volumeFlux_ana = (G/(2.0_real64*nu))*((4.0_real64*(W**3))/3.0_real64 + 4.0_real64*l*(W**2))
...
open (9 , file = "channelFlow-transient-navier.txt" , form = 'formatted')
...
!---------------------------------------------------------
open(unit=10, file='data2Plot-channelFlow-navier.dat',status='unknown')
24 format(100(ES18.7,3X))
do i = 1, n
  write(10,24) y(i), (data2Plot(i,j), j = 1, nPlots)
enddo
close(10)
!---------------------------------------------------------
