...
implicit none
!Number of nodes
integer(int64), parameter :: n = 251, maxIter = 1000000, nPlots=31
integer(int64) :: info , i, j, totalTimeSteps, timeStep4Plot, t, col
integer(int64), dimension ( n ) :: ipiv
real(real64), parameter :: mu = (9.6_real64)*1.0e-4, rho = 730.0_real64, & ! viscosity and density of decane
  W = (22.0_real64)*1.0e-9, &
  totalSimTime = (1.0_real64)*1.0e-9, timeStep = (2.0_real64)*1.0e-15, pi=4.0_real64*ATAN(1.0_real64)
...
do i =1 , n
  ...
  f_ana(i) = G/(4.0_real64*nu)*(W**2 - r(i)**2)
end do
!---------------------------------------------------------
...
do t = 1, totalTimeSteps
  ...
  if (temp(1)/=temp(1)) then
    print *, 'Flow value updated found to be NaN for index 1, at timestep', t
    print "(A3,2X,I5.5,2X,2(ES11.4,2X))", 't =', t, f(1), f(2)
    stop
  end if
  do i = 2, n-1
    temp(i) = source(i) + &
      A(i,i-1)*f(i-1) + A(i,i)*f(i) + A(i,i+1)*f(i+1)
    if (temp(i)/=temp(i)) then
      print *, 'Flow value updated found to be NaN for index ', i, ' at timestep', t
      print "(A3,2X,I7.7,2X,3(ES11.4,2X))", t, f(i-1), f(i), f(i+1)
      stop
    end if
  enddo
  temp(n) = f(n)
  ...
  if (modulo(t,timeStep4Plot) == 0) then 
    print "(A3,2X,I8,2X,2(ES11.4,2X))", 't =', t, maxval(f, 1), f(1)
    !----------------------------------------
    do i = 1, n
        data2Plot(i,col) = f(i)
    end do
...
! numeric volume flux:
volumeFlux_ana = (pi*G*(W**4))/(8.0_real64*nu)
...
open (9 , file = "pipeFlow-transient.txt" , form = 'formatted')
...
!---------------------------------------------------------
open(unit=10, file='data2Plot-pipeFlow.dat',status='unknown')
